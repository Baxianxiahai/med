...
class SEUI_L4_MainWindow(QtWidgets.QMainWindow, Ui_cebsMainWindow, clsL1_ConfigOpr):
    ...
    def initUI(self):
        ...
        aboutAction = QAction(
            QIcon('.\icon_res\cebsAbout.ico'), '&About', self)
        aboutAction.setShortcut('Ctrl+A')
        aboutAction.setStatusTip(
            'ABOUT SYSTEM - 上海小慧智能科技有限公司, 上海纳贤路800号科海大厦302-5')
        ...
        # 拍照
        self.actionMenuPicNorStart.triggered.connect(
            self.slot_ctrl_start_normal)
        ...
        # 识别
        self.actionMenuCfyNorStart.triggered.connect(
            self.slot_ctrl_vclas_start_normal)
        self.actionMenuCfyFluStart.triggered.connect(
            self.slot_ctrl_vclas_start_flu)
    ...
    # MUST Load global parameters, to initialize different UI and update the
    # stored parameters.
    def initParameter(self):
        # STEP1: START SUB-UI, 启动子界面
        self.instL4CalibForm = PkgL4ceuiHandler.ModCeuiCalib.SEUI_L4_CalibForm(
            self.TkCalibUi)
        self.instL4GparForm = PkgL4ceuiHandler.ModCeuiGpar.SEUI_L4_GparForm(
            self.TkGparUi)
        self.instL4MengForm = PkgL4ceuiHandler.ModCeuiMeng.SEUI_L4_MengForm(
            self.TkMengUi)
        self.instL4StestForm = PkgL4ceuiHandler.ModCeuiStest.SEUI_L4_StestForm(
            self.TkStestUi)
        self.instL4SahtForm = PkgL4ceuiHandler.ModCeuiSaht.SEUI_L4_SahtForm(
            self.TkSahtUi)
        self.instL4FspcForm = PkgL4ceuiHandler.ModCeuiFspc.SEUI_L4_FspcForm(
            self.TkFspcUi)

        # SIGNAL SLOPkgL4qtuiHandler
        # self.sgL4MainWinUnvisible.connect(self.funcMainWinUnvisible);
        self.sgL4MainWinVisible.connect(self.funcMainWinVisible)
        self.instL4CalibForm.sgL4MainWinVisible.connect(
            self.funcMainWinVisible)
        ...
        self.instL4StestForm.sgL4MainWinVisible.connect(
            self.funcMainWinVisible)
    ...
    def test_openMsg(self):
        file, ok = QFileDialog.getOpenFileName(
            self, "OPEN FILE", "C:/", "All Files (*);;Text Files (*.txt)")
    ...
    def slot_saht_sel(self):
        self.cetk_debug_print(
            "L4MAIN: Active Hole Selection function under construction......")
        if not self.instL4SahtForm.isVisible():
            self.sgL4MainWinUnvisible.emit()
    ...
    def slot_runpg_test(self):
        ...
        self.TkMainUi.funcPrintTestCalledByQt(
            "TEST! Send test msg to UI_MAIN and then send back to UI show by signal slot.")
