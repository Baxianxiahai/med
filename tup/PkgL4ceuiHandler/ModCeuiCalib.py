...
# 第二主入口
# Calibration Widget
class SEUI_L4_CalibForm({}, Ui_cebsCalibForm, clsL1_ConfigOpr):
    ...
    def initParameter(self):
        ...
        if (os.path.exists('calibInitWorm.jpg') == True):
            self.filePicInit = QtGui.QPixmap('calibInitWorm.jpg').scaled(
                self.calibRect.width(), self.calibRect.height())
...
#         self.instL3CalibProc.funcCalibPilotCameraEnable();
    ...
    def cetk_calib_disp_cam(self, fileName):
        if (os.path.exists(fileName) == True):
            temp_pixmap = QtGui.QPixmap(fileName).scaled(
                self.calibRect.width(), self.calibRect.height())
            self.label_calib_RtCam_Fill.setPixmap(temp_pixmap)

    def cetk_calib_disp_cam_by_obj(self, picObj):
        self.label_calib_RtCam_Fill.setPixmap(picObj.scaled(
            self.calibRect.width(), self.calibRect.height()))
