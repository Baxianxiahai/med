...
class tupTaskTemplate():
    ...
    def __init__(self, taskid, taskName, glTabEntry):
        ...
        self.state = 0
        self.ctrlRun = True
        self.msgStateMatrix = [[TUP_INIT_VALUE for i in range(
            self.glTab.TUP_MSGID_MAX)] for j in range(self.glTab.TUP_STATE_MAX)]
        self.task_create()
    ...
    def task_create(self):
        ...
        if (TUP_TASK_CUR_MOD == TUP_TASK_THREAD_MODE):
            self.process = threading.Thread(
                target=self.task_handler_enginee, args=(self.queue,))
        # 进程模式
        else:
    ...
    def task_handler_enginee(self, myque):
        while (self.ctrlRun == True):
            ...
            if (dstId != self.taskId):
                self.tup_err_print(
                    "Wrong destination module, self taskId = %d, dstId=%d." % (self.taskId, dstId))
                continue
            ...
            if self.msgStateMatrix[TUP_STM_COMN][msgId] != TUP_INIT_VALUE:
                ...
                if (proc(msgCont) == TUP_FAILURE):
                    self.tup_err_print(
                        "Proc execute error, Src/Dst/MsgId=%d/%d/%d, MsgContent=%s" % (srcId, dstId, msgId, str(msgCont)))
                continue
            # CASE2: NOT EXIST
            if self.msgStateMatrix[self.state][msgId] == TUP_INIT_VALUE:
                self.tup_err_print(
                    "Un-valid state-message set, inc State/MsgId=%d/%d" % (self.state, msgId))
                continue
    ...
    def tup_trace(self, string):
        if (TUP_PRINT_FLAG_SET_TRC == True):
            print(time.asctime(), " [", str(time.time()),
                  "] [TRC] [", self.taskName, "]: ", str(string))

    def tup_dbg_print(self, string):
        if (TUP_PRINT_FLAG_SET_DBG == True):
            print(time.asctime(), ", [DBG] [",
                  self.taskName, "]: ", str(string))
    ...
    def tup_get_from_redis(self, field, key):
        redishost = os.environ['REDISHOST']
        if redishost is None:
            redishost = '127.0.0.1'
        rdp = redis.ConnectionPool(
            host=redishost, port=6379, password='', decode_responses=True)
        rdc = redis.StrictRedis(connection_pool=rdp)
        return rdc.hget(field, key)
